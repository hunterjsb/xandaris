...
// FleetCommandExecutor handles fleet movement commands
type FleetCommandExecutor struct {
	systems    []*entities.System
	hyperlanes []entities.Hyperlane
}

// NewFleetCommandExecutor creates a new fleet command executor
func NewFleetCommandExecutor(systems []*entities.System, hyperlanes []entities.Hyperlane) *FleetCommandExecutor {
	return &FleetCommandExecutor{
		systems:    systems,
...
func (fce *FleetCommandExecutor) MoveFleetToSystem(fleet *entities.Fleet, targetSystemID int) (int, int) {
	...
	for _, system := range fce.systems {
		systemsMap[system.ID] = system
	}
	helper := tickable.NewShipMovementHelper(systemsMap, fce.hyperlanes)
...
func (fce *FleetCommandExecutor) MoveFleetToPlanet(fleet *entities.Fleet, targetPlanet *entities.Planet) (int, int) {
...
func (fce *FleetCommandExecutor) MoveFleetToStar(fleet *entities.Fleet) (int, int) {
	if fleet == nil || len(fleet.Ships) == 0 {
		return 0, 0
	}
...
func (fce *FleetCommandExecutor) GetConnectedSystems(fromSystemID int) []int {
	connected := make([]int, 0)

	for _, hyperlane := range fce.hyperlanes {
...
func (fce *FleetCommandExecutor) GetSystemByID(systemID int) *entities.System {
	for _, system := range fce.systems {
