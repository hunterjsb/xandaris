...
func seedNewSchema(app *pocketbase.PocketBase) error {
	...
	// 9. Convert map lanes to hyperlanes
	if err := convertLanesToHyperlanes(app); err != nil {
		return fmt.Errorf("failed to convert lanes to hyperlanes: %w", err)
	}
	...
	fmt.Printf("  - Map lanes converted to hyperlanes\n")
...
func convertLanesToHyperlanes(app *pocketbase.PocketBase) error {
	...
	fmt.Printf("Converting %d lanes to hyperlanes...\n", len(lanes))
	...
	fmt.Printf("✅ Converted %d lanes to hyperlanes\n", hyperlaneCount)
	return nil
}
