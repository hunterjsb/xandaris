    ...
    `}renderMovingFleet(e,t){const s=e.name||`Fleet ${e.id.slice(-4)}`,i=this.gameState.systems||[],n=this.gameState.currentTick||0,a=60/(this.gameState.ticksPerMinute||6),r=i.find(S=>S.id===e.current_system),l=r?r.name:"Deep Space",c=i.find(S=>S.id===t.destination_system_id),h=c?c.name:"Unknown System",d=Math.max(0,t.execute_at_tick-n),u=(d*a).toFixed(0);let p=`${d} ticks (~${u}s)`;d===0&&t.status==="processing"?p="Finalizing Jump":d===0&&t.status==="pending"&&(p="Initiating Jump");const f=t.travel_time_ticks||2,v=f-d,g=Math.round(v/f*100),y=t.status.charAt(0).toUpperCase()+t.status.slice(1);let x="";if(t.route_path&&t.route_path.length>2){const S=t.current_hop||0,C=t.route_path.length-1,_=C-S,P=i.find(R=>R.id===t.final_destination_id);x=`
        <div class="border-t border-space-500 mt-2 pt-2 text-xs">
          <div class="grid grid-cols-2 gap-2">
            ...
            <div><span class="text-space-400">Remaining:</span> <span class="text-yellow-400">${_} hops</span></div>
          </div>
        </div>
      `}return`
    ...
    `}getShipTypeData(){return(this.gameState.shipTypes||[]).find(t=>t.id===this.ship.ship_type||t.name===this.ship.ship_type_name)}getShipCargo(){var s;const e=(s=this.gameState)==null?void 0:s.getFleetCargo(this.fleet.id);if(!e||!e.cargo)return[];const t=[];for(const[i,n]of Object.entries(e.cargo))n>0&&t.push({ship_id:this.ship.id,resource_name:i,resource_type:i,quantity:n});return t}getResourceDensity(e){return{ore:2.5,metal:7.8,fuel:.8,food:1.2,water:1,machinery:3.2,electronics:1.5}[e==null?void 0:e.toLowerCase()]||1}getResourceValue(e){return{ore:10,metal:25,fuel:15,food:8,water:5,machinery:100,electronics:200}[e==null?void 0:e.toLowerCase()]||1}isFleetMoving(){return(this.gameState.fleetOrders||[]).some(t=>t.fleet_id===this.fleet.id&&(t.status==="pending"||t.status==="processing"))}}class Qe{constructor(e,t){this.uiController=e,this.gameState=t,this.fleetListComponent=new Xe(e,t),this.fleetComponent=new Ke(e,t),this.shipComponent=new Je(e,t),this.shipCargoComponent=new Ze(e,t),this.currentView="list",this.currentFleetId=null,this.currentShipId=null,window.fleetComponents=this}showFleetPanel(){this.currentView="list",this.currentFleetId=null,this.currentShipId=null,this.uiController.showFleetPanelAsFloating()}showFleetDetails(e){this.currentView="fleet",this.currentFleetId=e,this.currentShipId=null,this.uiController.showFleetDetailsAsFloating(e)}showShipDetails(e){this.currentView="ship",this.currentShipId=e,this.uiController.showShipDetailsAsFloating(e)}showShipCargo(e){this.currentView="cargo",this.currentShipId=e,this.uiController.showShipCargoAsFloating(e)}back(){switch(this.currentView){case"cargo":this.currentShipId?this.showShipDetails(this.currentShipId):this.showFleetPanel();break;case"ship":this.currentFleetId?this.showFleetDetails(this.currentFleetId):this.showFleetPanel();break;case"fleet":this.showFleetPanel();break;default:this.uiController.hideModal();break}}backToFleet(e){this.showFleetDetails(e)}backToShip(e){this.showShipDetails(e)}sendFleet(e){var s;const t=(s=this.gameState.fleets)==null?void 0:s.find(i=>i.id===e);if(t){const n=(this.gameState.systems||[]).find(o=>o.id===t.current_system);n&&(this.uiController.hideModal(),this.uiController.showSendFleetModal(n))}}manageFleet(e){this.uiController.showToast("Fleet management coming soon!","info",3e3)}viewCargo(e){var s;const t=(s=this.gameState.fleets)==null?void 0:s.find(i=>i.id===e);if(t&&t.ships&&t.ships.length>0){const i=t.ships.find(n=>{const o=this.getShipTypeData(n);return(o==null?void 0:o.cargo_capacity)>0});i?this.showShipCargo(i.id):this.uiController.showToast("No cargo ships found in this fleet","info",3e3)}else this.uiController.showToast("Fleet has no ships","error",3e3)}transferCargo(e,t){this.showCargoTransferModal(e,t)}showCargoTransferModal(e,t){var c,h,d,u;const s=(c=this.gameState.fleets)==null?void 0:c.find(p=>p.id===e),i=(h=this.gameState.systems)==null?void 0:h.find(p=>p.id===t);if(!s||!i){this.uiController.showToast("Fleet or system not found","error",3e3);return}const n=((d=this.gameState)==null?void 0:d.getFleetCargo(e))||{cargo:{},used_capacity:0,total_capacity:0},o=((u=this.gameState)==null?void 0:u.playerResources)||{},a=Object.entries(n.cargo||{}).filter(([p,f])=>f>0).map(([p,f])=>({resource:p,quantity:f,type:"fleet"})),r=Object.entries(o).filter(([p,f])=>f>0&&p!=="credits").map(([p,f])=>({resource:p,quantity:f,type:"player"})),l=this.renderCargoTransferModal(s,i,a,r,n);this.uiController.showModal(`Cargo Transfer: ${s.name||`Fleet ${s.id.slice(-4)}`} ↔ ${i.name}`,l)}renderCargoTransferModal(e,t,s,i,n){const o=e.name||`Fleet ${e.id.slice(-4)}`,a=t.name||`System ${t.id.slice(-4)}`,r=s.length>0?s.map(c=>this.renderCargoItem(c,"unload",e.id)).join(""):'<div class="text-space-400 text-center py-4">Fleet cargo is empty</div>',l=i.length>0?i.map(c=>this.renderCargoItem(c,"load",e.id)).join(""):'<div class="text-space-400 text-center py-4">No planetary resources available</div>';return`
      <div class="cargo-transfer-modal space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <!-- Planet Resources (Load) -->
          <div class="cargo-section">
            <h3 class="text-lg font-semibold mb-3 text-nebula-200 flex items-center gap-2">
              <span class="material-icons text-sm">public</span>
              Planet Resources
              <span class="text-xs text-space-400 ml-auto">Loading disabled</span>
            </h3>
            <div class="space-y-2 max-h-64 overflow-y-auto custom-scrollbar">
              ${l}
            </div>
        ...
        <div class="mt-6 pt-4 border-t border-space-600">
          <div class="text-xs text-space-400 mb-3">
            ...
            Click fleet cargo items to unload them to planetary storage. Loading from planets coming soon!
          </div>
